...
<html>
	...
	<body style="background: #FFF">
					...
					<div class="padding: 15px;" class = "components_alfa">
						...
						<div class = "components_text">
                                          ...
                                          <?php
                                            ...
                                            while($row = $results->fetch_assoc()) {
                                                if($row['ip'] != "" & $row['mID'] != "" & $row['ip'] != "localhost"){
                                                    echo "<option  name='select' value=".$row['mID'].">".$row['mID']."</option>";
                                                }
                                            }
                                          ?>
                                      </select>
                            ...
                            <div>
                                ...
                                <a class="tooltip noselect DataV"><p>Текущая температура на улице: <?echo $temper1;?>С</p><span><img src="grafick.php?ip=192.168.1.11&type=temp&start=<?echo $seldays1[0];?>&period=<?echo $seldays1[2];?>"/></span></a>
								...
								<script>
								function autoload(){
								   postAjax('http://<?echo $_SERVER['HTTP_HOST'];?>/michome/prognoz.php?modD=sborinfo_tv&modT=termometr_okno', "GET", "", function(d){document.getElementById("prognoz").innerHTML = d;});
								}
								window.setTimeout("autoload()",1);
								</script>
		...
		<?php require_once("../site/verhn.php");?>       
	</body>
</html>	
