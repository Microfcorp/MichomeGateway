...
$refCount = 0;
$res = mysqli_query($link, "SELECT  COUNT(`id`) AS `cnt` FROM `michome`.`michom` WHERE `ip`='$ip' AND `type`='$type' AND `date` >= NOW() - INTERVAL 2 SECOND ORDER BY `id` DESC");
while($row = $res->fetch_assoc()) {
	$refCount = $row['cnt'];
}

if($refCount >= intval($API->standartCountRequests)){
	$JSONData['errorCode'] = 6;
	$JSONData['error'] = "This module is many requests";
	exit(json_encode($JSONData));
}
	...
	elseif($type == "meteostation"){//Метеостация
		...
		for($i = 0; $i < count($meteo); $i++){		
			if(intval($meteo[$i][0]) > 1024 || intval($meteo[$i][1]) > 1024 || intval($meteo[$i][0]) < -100 || intval($meteo[$i][1]) < 0){				
				$API->AddLog($ModuleID, $type, $rsid, "Text=Error validation MeteoStation data. Sensors: {$i}, Data: ".implode(', ', $meteo[$i]).";", $date);
				continue;
			}
