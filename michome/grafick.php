...
//mode=webp png jpg gif ИЛИ mode=map - Формат выходного изображения или json карта координат (по умолчанию png)
...
$mode = isset($_GET['mode']) ? $_GET['mode'] : "webp";
...
if(count(array_filter($data)) < 1){
	...
	elseif($mode == "png" || $mode == "jpg" || $mode == "gif" || $mode == "webp"){
...
if($mode == "png" || $mode == "jpg" || $mode == "gif" || $mode == "webp"){
	//создаем изображение
	$im = @ImageCreate ($width, $height) or die ("Cannot Initialize new GD image stream");
...
{
  $i = round($i, 1);
  $y = round($gheight - ($i - $min) * ($gheight) / ($max - $min) + $padding);
  if($mode == "png" || $mode == "jpg" || $mode == "gif" || $mode == "webp"){
	  ImageLine($im, $padding, $y, round($gwidth + 1.5 * $padding), round($y), $gray);
	  ImageTTFText($im, 8, 0, round($padding + 1), $y - 1, $gray, __DIR__."//../site/Verdana.ttf", $i);
  }
}
...
{
  $x1 = round($x2);
  $x2 = round($x1 + (($gwidth) / ($cnt == 1 ? 1 : ($cnt - 1))));
  $y1 = round($y2);
  $y2 = round($gheight - ($data[$i] - $min) * ($gheight) / ($max - $min) + $padding);     
